...
// Mock the secure session module
...
describe('AuthStore', () => {
  beforeEach(() => {
    ...
    (secureSession as any)._clearMockStorage?.();
    // Reset all mocks
    vi.clearAllMocks();
    authStore.logout(); // Reset state after clearing mocks
  });
  ...
  describe('unlockVault', () => {
    ...
    it('should throw error if no identity loaded', async () => {
      ...
      await expect(authStore.unlockVault('any-passphrase')).rejects.toThrow('No identity loaded');
    });
  ...
  describe('logout', () => {
    ...
    it('should clear localStorage and sessionStorage', () => {
      // Set some data in localStorage to test clearing
      localStorage.setItem('volli-identity', 'test-identity');
  ...
  describe('checkSession', () => {
    ...
    it('should restore session from sessionStorage', async () => {
      ...
      // Mock secure session to return valid session
      secureSession.get.mockReturnValue({
        token: savedToken!,
      ...
      // Check session should restore from sessionStorage
      const hasSession = await authStore.checkSession();
    ...
    it('should clear expired sessions', async () => {
      ...
      secureSession.get.mockReturnValueOnce(null); // Expired session returns null
      ...
      expect(hasSession).toBe(false);
      expect(secureSession.get()).toBe(null);
    });
  });

  describe('Session restoration error handling', () => {
    it('should handle corrupted session data gracefully', async () => {
      // Mock corrupted session that returns null
      secureSession.get.mockReturnValueOnce(null);
    ...
    it('should handle corrupted identity data gracefully', async () => {
      // Mock valid session
      secureSession.get.mockReturnValueOnce({
        token: 'test-token',
        expiresAt: Date.now() + 10000,
        createdAt: Date.now(),
      });
  ...
  describe('Initialize function', () => {
    it('should restore session from sessionStorage on initialize', async () => {
      // Setup: Create a session
      await authStore.createIdentity('Test User');
      await authStore.createVaultWithPassphrase(fixtures.validPassphrase);
      ...
      // Call initialize - should restore from sessionStorage
  ...
  describe('Passphrase validation', () => {
    ...
    it('should detect common patterns', () => {
      const commonPatterns = ['password123', '12345678', 'qwertyuiop', 'admin1234'];
